...
\begin{lstlisting}[language=C++, caption=GF Multiplication by 2, breaklines=true]
...
\begin{lstlisting}[language=C++, caption=AddRoundKey, breaklines=true]
for (int i = 0; i < 16; i++) {
...
\begin{lstlisting}[language=C++, caption=AES-128 Encryption with AES-NI, breaklines=true, basicstyle=\footnotesize\ttfamily]
...
\begin{lstlisting}[language=C++, caption=Key Expansion with AES-NI Helper, breaklines=true, basicstyle=\footnotesize\ttfamily]
...
\begin{lstlisting}[language=C++, caption=From encrypt\_aesni.cpp, breaklines=true, basicstyle=\footnotesize\ttfamily]
...
\begin{itemize}
    \item Software encryption $\rightarrow$ Hardware decryption: PASS
    \item Hardware encryption $\rightarrow$ Software decryption: PASS
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{stat_histogram.pdf}
\caption{Hamming distance distribution for tweak increments (2.55M measurements). Red line: observed mean (64.00 bits); green line: theoretical expectation (64.0 bits). Distribution follows binomial B(128, 0.5).}
...
\begin{itemize}
    ...
    \item Timing method: \texttt{clock\_gettime()} with \texttt{CLOCK\_MONOTONIC} (nanosecond precision)
...
\textbf{Results}:
\begin{verbatim}
[PASS] AES-128 software vs hardware without tweak
[PASS] AES-128 software vs hardware with tweak
[PASS] AES-192 software vs hardware without tweak
[PASS] AES-192 software vs hardware with tweak
[PASS] AES-256 software vs hardware without tweak
[PASS] AES-256 software vs hardware with tweak
...
\textbf{Results}:
\begin{verbatim}
[PASS] AES-128 software wrong key (correctly fails)
[PASS] AES-128 software wrong key with tweak (correctly fails)
[PASS] AES-128 hardware wrong key (correctly fails)
[PASS] AES-128 hardware wrong key with tweak (correctly fails)
... (similar for 192, 256)
...
\textbf{Results}:
\begin{verbatim}
[PASS] AES-128 software wrong tweak (correctly fails)
[PASS] AES-192 software wrong tweak (correctly fails)
[PASS] AES-256 software wrong tweak (correctly fails)
[PASS] AES-128 hardware wrong tweak (correctly fails)
[PASS] AES-192 hardware wrong tweak (correctly fails)
[PASS] AES-256 hardware wrong tweak (correctly fails)
...
\textbf{Results}:
\begin{verbatim}
[PASS] AES-128 software irregular size (ciphertext stealing)
[PASS] AES-128 software irregular size with tweak (ciphertext stealing)
[PASS] AES-128 hardware irregular size (ciphertext stealing)
[PASS] AES-128 hardware irregular size with tweak (ciphertext stealing)
... (similar for 192, 256)
