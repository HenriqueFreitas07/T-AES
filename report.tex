...
% for the whole project, not abstract when we refer something or a program we should refer the name of the script for example if were talking about our aes we say we have the structure in aes.hpp and call it in encrypt, we just need to refer each one once, for example also speaking about the speed we should say what it does and the script speed.cpp and how to call it ./bin/speed and also the make file we have
% all the -- that are used instead of commas shoould be removed for example AES-NI is correct but having equivalent-given in the place of a coma is wrong
% we have 6 pages at the moment, maybe we can put snipets of the code in some places when explaining the code, and also explain a bit more the theory behind it

% include that we are implementing AES in cpp with a tweak and explain briefly the tweak in abstract, arythmetic adition
% refer that we also provide togehter with that the two functinos speed and stat where we prove the things given to us in the pdf explaining the work
% explain that we also compare with xts but ofc in an abstract way
\begin{abstract}
This report presents T-AES, a tweakable Advanced Encryption Standard implementation featuring both software and hardware-accelerated (AES-NI) versions. The tweak, a 128-bit value combined via arithmetic addition (mod $2^{128}$) with the middle round key, enables deterministic variation across encrypted blocks without requiring secrecy, similar in concept to XTS mode. The implementation supports AES-128, AES-192, and AES-256 in an ECB-based counter mode where the tweak increments per block. Ciphertext stealing handles non-block-aligned data without padding. The project includes encrypt and decrypt command-line applications, a speed benchmark application (speed.cpp) comparing T-AES software, T-AES AES-NI, and OpenSSL XTS implementations, and a statistical analysis application (stat.cpp) that measures Hamming distance distributions to validate the cryptographic properties of the tweak. Both T-AES implementations are validated to produce identical outputs, demonstrating cryptographic equivalence.
\end{abstract}
...
% replace the facto for something else
% refer xts and cts and the tweak and xtx 128 and 256
...
% and also achieving cross encryptino and decryption with of course the same key and tweak if done with tht tweak
\section{Baseline Requirements and Specifications}

The T-AES project was designed with clear functional and implementation requirements to ensure modularity, performance, and strict adherence to the AES standard. The goal was to create a command-line tool capable of encrypting and decrypting data streams using both software and hardware-accelerated versions of AES, while maintaining identical cryptographic behavior in both implementations.


% if not exmplained further explain a bit better what each thing is, careful not to go over 8 pages, the lokoup tables, what are the bitwise operations, but i think they are written after
...
\vspace{0.05cm}
\begin{lstlisting}[language=C++, caption=Counter Mode Tweak Increment, label=lst:tweak_increment, basicstyle=\scriptsize\ttfamily, breaklines=true]
...
Testing is organized into six stages with corresponding test groups. The core validation logic (shown in Listing~\ref{lst:test_core}) encrypts plaintext, decrypts the ciphertext, and verifies byte-for-byte equivalence:
...
The stat.cpp application (invoked via \texttt{./bin/stat}) validates the cryptographic properties introduced by the tweak mechanism. By encrypting the same plaintext with incrementing tweak values over 10,000 experiments with 256 tweaks each (2.5M+ Hamming distance measurements), the application measures how tweak variations propagate through the ciphertext. This empirically validates the diffusion properties required of a tweakable cipher.
...
All original AES core implementation, tweak integration, ciphertext stealing logic, and counter-mode operations are authored as part of this project. Intermediate testing functions (\texttt{verify\_aes.cpp}, etc.) developed during implementation are no longer required as the \texttt{test\_comprehensive.sh} script and manual CLI tests provide comprehensive validation.
