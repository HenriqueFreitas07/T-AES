...
void AES::ShiftRows(vector<vector<uint8_t>> &matrix) {
    ...
    // The objective of ShiftRows is to cyclically shift the rows of the state matrix
    // So that each byte is moved to a different position
    ...
    for (size_t r = 1; r < matrix.size(); ++r) {
        ...
        // }
        rotate(matrix[r].begin(), matrix[r].begin() + r, matrix[r].end()); // can use rotate instead
    }
}
...
void AES::MixColumns(vector<vector<uint8_t>> &matrix) {
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        matrix[0][c] = xtime(s0) ^ (xtime(s1) ^ s1) ^ s2 ^ s3; // 2*s0 + 3*s1 + s2 + s3
        matrix[1][c] = s0 ^ xtime(s1) ^ (xtime(s2) ^ s2) ^ s3; // s0 + 2*s1 + 3*s2 + s3
        matrix[2][c] = s0 ^ s1 ^ xtime(s2) ^ (xtime(s3) ^ s3); // s0 + s1 + 2*s2 + 3*s3
        matrix[3][c] = (xtime(s0) ^ s0) ^ s1 ^ s2 ^ xtime(s3); // 3*s0 + s1 + s2 + 2*s3
    }
}
