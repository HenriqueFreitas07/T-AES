...
\title{T-AES: Software and Hardware-Accelerated AES Implementation\\with Ciphertext Stealing}

\author{\IEEEauthorblockN{Advanced Cryptography Project}
...
This project implements AES encryption and decryption in software and hardware-accelerated forms, in order to compare both approaches in terms of permformance. The implementation follows NIST FIPS 197 specifications and includes support for all three standard key sizes (128, 192, and 256 bits). A notable feature is the ciphertext stealing technique, which eliminates the need for padding when processing data that doesn't align to 16-byte block boundaries.
...
When input data length is not a multiple of 16 bytes, the implementation must use ciphertext stealing rather than padding:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Functional Equivalence}: Both versions must produce identical ciphertext for identical inputs, hand both must work together and interleave between the outputs and inputs of the cipherstreams.
...
SubBytes provides non-linearity by replacing each byte with its corresponding value from the AES S-box.
...
\begin{itemize}
    ...
    \item Simple array indexing: \texttt{state[i] = sbox[state[i]]}
\end{itemize}
...
MixColumns provides strong diffusion by mixing bytes within each column using Galois Field arithmetic.
...
\begin{lstlisting}[language=C++, caption=GF Multiplication by 2]
...
AddRoundKey introduces key material by XORing the state with the round key.

\textbf{Implementation}:
\begin{lstlisting}[language=C++, caption=AddRoundKey]
for (int i = 0; i < 16; i++) {
...
\begin{itemize}
    \item Intrinsic: \texttt{\_mm\_aesenc\_si128(\_\_m128i state, \_\_m128i roundkey)}
...
\begin{itemize}
    \item Intrinsic: \texttt{\_mm\_aesenclast\_si128(\_\_m128i state, \_\_m128i roundkey)}
...
\begin{itemize}
    \item Intrinsic: \texttt{\_mm\_aesdec\_si128(\_\_m128i state, \_\_m128i roundkey)}
...
\begin{itemize}
    \item Intrinsic: \texttt{\_mm\_aesdeclast\_si128(\_\_m128i state, \_\_m128i roundkey)}
...
\begin{itemize}
    \item Intrinsic: \texttt{\_mm\_aeskeygenassist\_si128(\_\_m128i key, const int rcon)}
...
\begin{lstlisting}[language=C++, caption=AES-128 Encryption with AES-NI]
...
\begin{lstlisting}[language=C++, caption=Key Expansion with AES-NI Helper]
void aesni_key_expansion_128(uint8_t* userkey,
    ...
    key = key_expansion_128(key,
          _mm_aeskeygenassist_si128(key, 0x02));
    key_schedule[2] = key;

    // Continue for remaining 8 rounds...
}
...
\begin{itemize}
    ...
    \item \textbf{Parallelism}: Dedicated circuits perform SubBytes, ShiftRows, MixColumns simultaneously
...
\begin{lstlisting}[language=C++, caption=From encrypt\_aesni.cpp]
...
for (size_t i = 0; i < all_blocks.size(); i++) {
    ...
    if (current_block.size() < 16) {
        // Ciphertext stealing logic (same as software)
        // ...
        ciphertext_block = aes_ni.encrypt_block(current_block);
    } else {
        // Normal encryption with AES-NI
        ciphertext_block = aes_ni.encrypt_block(current_block);
    }
...
The automated test harness executes 150+ individual tests organized into six categories:
\begin{itemize}
...
\begin{itemize}
    \item Software encryption → Hardware decryption: ✓
    \item Hardware encryption → Software decryption: ✓
...
The \texttt{stat} application measures the cryptographic avalanche property of the tweak modification. By incrementing the tweak and measuring Hamming distances between successive ciphertexts, we evaluate how well the tweak contributes to diffusion.
...
\begin{enumerate}
    ...
    \item Encrypt with tweak = 0, store ciphertext C₀
    \item For tweak = 1 to 255:
    \begin{itemize}
        \item Encrypt same plaintext with new tweak → Cᵢ
        \item Compute Hamming distance: HD(Cᵢ, Cᵢ₋₁)
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{stat_histogram.pdf}
\caption{Hamming distance distribution for tweak increments (2.55M measurements). The red dashed line indicates the observed mean (64.00 bits), and the green dashed line shows the theoretical expectation (64.0 bits). The distribution follows a binomial B(128, 0.5), demonstrating perfect avalanche effect.}
...
\begin{itemize}
    ...
    \item Timing method: \texttt{clock\_gettime(CLOCK\_MONOTONIC)} (nanosecond precision)
...
\hline
\textbf{Operation} & \textbf{Min (μs)} & \textbf{Avg (μs)} & \textbf{Max (μs)} & \textbf{Throughput (GB/s)} \\
\hline
...
\begin{itemize}
    ...
    \item Average latency: 8-13 μs per 4KB block
...
\begin{itemize}
    \item Pure encryption: ~2 μs (AES-NI)
    \item Memory operations: ~3 μs
    \item Overhead (setup, checks): ~3 μs
    \item Total: ~8 μs per 4KB block
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{AES-128 total}: 10 rounds × 132 + overhead ≈ 1400 instructions
\end{itemize}
...
\begin{itemize}
    ...
    \item Load/store: 2 × \_mm\_loadu\_si128, 1 × \_mm\_storeu\_si128
...
\textbf{Results}:
\begin{verbatim}
✓ AES-128 software without tweak
✓ AES-128 software with tweak
✓ AES-128 hardware without tweak
✓ AES-128 hardware with tweak
✓ AES-192 software without tweak
✓ AES-192 software with tweak
✓ AES-192 hardware without tweak
✓ AES-192 hardware with tweak
✓ AES-256 software without tweak
✓ AES-256 software with tweak
✓ AES-256 hardware without tweak
✓ AES-256 hardware with tweak
...
cmp -s plaintext.bin output.bin  # Binary comparison
\end{lstlisting}
...
\textbf{Results}:
\begin{verbatim}
✓ AES-128 software vs hardware without tweak
✓ AES-128 software vs hardware with tweak
✓ AES-192 software vs hardware without tweak
✓ AES-192 software vs hardware with tweak
✓ AES-256 software vs hardware without tweak
✓ AES-256 software vs hardware with tweak
...
\textbf{Results}:
\begin{verbatim}
✓ AES-128 software wrong key (correctly fails)
✓ AES-128 software wrong key with tweak (correctly fails)
✓ AES-128 hardware wrong key (correctly fails)
✓ AES-128 hardware wrong key with tweak (correctly fails)
... (similar for 192, 256)

Tests passed: 12/12 (negative tests)
\end{verbatim}
...
if ! cmp -s plaintext.bin output.bin; then
    echo "PASS (correctly fails to decrypt)"
fi
\end{lstlisting}
...
\textbf{Results}:
\begin{verbatim}
✓ AES-128 software wrong tweak (correctly fails)
✓ AES-192 software wrong tweak (correctly fails)
✓ AES-256 software wrong tweak (correctly fails)
✓ AES-128 hardware wrong tweak (correctly fails)
✓ AES-192 hardware wrong tweak (correctly fails)
✓ AES-256 hardware wrong tweak (correctly fails)

Tests passed: 6/6 (negative tests)
\end{verbatim}
...
\textbf{Results}:
\begin{verbatim}
✓ AES-128 software irregular size (ciphertext stealing)
✓ AES-128 software irregular size with tweak (ciphertext stealing)
✓ AES-128 hardware irregular size (ciphertext stealing)
✓ AES-128 hardware irregular size with tweak (ciphertext stealing)
... (similar for 192, 256)
...
\begin{lstlisting}[language=bash]
INPUT_SIZE=$(wc -c < irregular.txt)   # 23 bytes
cat irregular.txt | ./encrypt 128 "pwd" > cipher.bin
CIPHER_SIZE=$(wc -c < cipher.bin)     # 23 bytes
...
\bibitem{fips197} National Institute of Standards and Technology, ``Advanced Encryption Standard (AES),'' FIPS PUB 197, November 2001. Available: \url{https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197.pdf}

\bibitem{aeswiki} ``Advanced Encryption Standard,'' Wikipedia, The Free Encyclopedia. Available: \url{https://en.wikipedia.org/wiki/Advanced_Encryption_Standard}

\bibitem{intelwp} Intel Corporation, ``Advanced Encryption Standard (AES) Instructions Set,'' White Paper, September 2012. Available: \url{https://www.intel.com/content/dam/develop/external/us/en/documents/aes-wp-2012-09-22-v01-165683.pdf}

\bibitem{aesni} Intel Corporation, ``Intel Advanced Encryption Standard Instructions (AES-NI),'' 2010. Available: \url{https://www.intel.com/content/www/us/en/developer/articles/technical/advanced-encryption-standard-instructions-aes-ni.html}

\bibitem{gueron} Shay Gueron, ``Intel Advanced Encryption Standard (AES) New Instructions Set,'' Intel Software Network, 2010. Available: \url{https://www.intel.com/content/dam/doc/white-paper/advanced-encryption-standard-new-instructions-set-paper.pdf}

\bibitem{sergeyaes} Sergey Bel, ``AES Implementation,'' GitHub Repository. Available: \url{https://github.com/SergeyBel/AES}

\bibitem{cts} National Institute of Standards and Technology, ``Recommendation for Block Cipher Modes of Operation: Three Variants of Ciphertext Stealing for CBC Mode,'' NIST Special Publication 800-38A Addendum, October 2010.

\bibitem{openssl} OpenSSL Software Foundation, ``OpenSSL: Cryptography and SSL/TLS Toolkit.'' Available: \url{https://www.openssl.org/}

\bibitem{daemen} Joan Daemen and Vincent Rijmen, ``The Design of Rijndael: AES - The Advanced Encryption Standard,'' Springer-Verlag, 2002.

\bibitem{nistmodes} National Institute of Standards and Technology, ``Recommendation for Block Cipher Modes of Operation,'' NIST Special Publication 800-38A, 2001 Edition.
\end{thebibliography}
